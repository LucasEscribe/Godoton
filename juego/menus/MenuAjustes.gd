...
onready var opciones_resolucion = $ContenedorTabulador/AudioVideo/PanelPrincipal/ContenedorPrincipal/Resolucion/OpcionResolucion
...
func _on_OpcionResolucion_item_selected(index: int) -> void:
	#print("Indice: {ind} - Res: {tx}".format({"ind": index, "tx": opciones_resolucion.get_item_text(index)}))
